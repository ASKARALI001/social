...
import {dyse, months, years} from "../../utils/birthday";
...
const Friends = () => {
	...
	const registerUser = data => {
		const { passwordAgain, months, year, day, ...other } = data
						...
						<div className='register__block'>
							<label className='register__label'>
								<h2 className='register__label-title'>{t('form.labelAge')}</h2>
									<select {...register('day')}className='editMyProfile__filed' >
										{
											dyse.map((item) => (
									...
									<select {...register('months')} className='editMyProfile__filed '>
										{
											months.map((item) => (
									...
									<select {...register('years')} className='editMyProfile__filed '>
										{
											years(2009).map((item) => (
												<option key={item} value={item}>{item}</option>
											))
